/*=============================================================================
    Fragment.glsl: Flu fragment shader.
...
//
// An information about post-effect.
//
struct TPostEffect
{
	vec3	Highlights;
...
void main()
{
	if( bRenderLightmap == 0 )
	{
		...
		if( bUnlit == 1 )
		{
			...
			vec3 Result	= pow( max( vec3(0,0,0), (Source.xyz-Effect.Shadows) )*(Effect.Highlights), Effect.MidTones );	

			float Des	= dot( Result, vec3(0.299, 0.587, 0.114) );
			Result		= mix( Result, vec3( Des, Des, Des ), Effect.BWScale );

			gl_FragColor	= vec4( Result, Source.w );
		}
		else
		{
			//
			...
			vec3 Result	= pow( max( vec3(0,0,0), (Color-Effect.Shadows) )*(Effect.Highlights), Effect.MidTones );	
	...
	else
	{
		...
		// Render lightmap.
		//
		vec3 Result = LightValue( true ) * Effect.Highlights;
		float Des	= dot( Result, vec3(0.299, 0.587, 0.114) );
		Result		= mix( Result, vec3( Des, Des, Des ), Effect.BWScale );
