...
public:
	// Functions.
	virtual void SerializeData( void* Mem, DWord Count ) = 0;
	virtual void SerializeRef( FObject*& Obj ) = 0;

	virtual DWord TotalSize()
	{
		return 0;
	}
	virtual void Seek( DWord NewPos )
	{
	}
	virtual DWord Tell()
	{
...
template<class T> inline void SerializeEnum( CSerializer& S, T& Enum )	{ S.SerializeData( &Enum, sizeof(Byte) ); }
