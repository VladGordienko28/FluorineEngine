...
//
CMemPool		CCanvas::GPool( L"Render", 4*1024*1024 );
...
)
{
	...
	else
	{
		...
		List.Bitmap		= Font->Bitmaps.Num() ? Font->Bitmaps[0] : nullptr;
...
// coords system.
//
void TViewInfo::Project( const TVector V, Float& OutX, Float& OutY )
{
	TVector R, PixToFOV;
...
// coords system.
//
TVector TViewInfo::Deproject( Float InX, Float InY )
{
	TVector R, FOVToPix;
