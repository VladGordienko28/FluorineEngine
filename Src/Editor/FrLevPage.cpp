...
void WLevelPage::OnDragOver( void* Data, Integer X, Integer Y, Bool& bAccept )
{
	...
	if( Data )
	{
		...
		// Bitmap drop.
		if( Object->IsA(FBitmap::MetaClass) )
		{
			FEntity* Entity = GetEntityAt( X, Y, false );
			if( Entity )
			{
				CProperty* Prop = Entity->Base->GetClass()->FindProperty(L"Bitmap");
				bAccept = Prop && TYPE_BITMAP.MatchTypes(*Prop);
			}
		}
	}
}
...
void WLevelPage::OnDragDrop( void* Data, Integer X, Integer Y )
{
	...
	else if( Obj->IsA(FBitmap::MetaClass) )
	{
		// Put bitmap on brush/model or something.
			FEntity* Entity = GetEntityAt( X, Y, false );
			if( Entity )
			{
				CProperty* Prop = Entity->Base->GetClass()->FindProperty(L"Bitmap");
				if( Prop && TYPE_BITMAP.MatchTypes(*Prop) )
				{
					Transactor->TrackEnter();
						*(FBitmap**)(((Byte*)Entity->Base)+Prop->Offset) = As<FBitmap>(Obj);
...
void WLevelPage::OnMouseDrag( EMouseButton Button, Integer X, Integer Y, Integer DeltaX, Integer DeltaY )
{
	...
	TVector Delta;
	TVector FOV	= Level->Camera->GetFitFOV( Size.Width, Size.Height );
	Delta.X		= +DeltaX * (FOV.X / Size.Width ) * Level->Camera->Zoom;
	Delta.Y		= -DeltaY * (FOV.Y / Size.Height) * Level->Camera->Zoom;
	Delta		= TransformVectorBy( Delta, Level->Camera->ToWorld() );
...
void WLevelPage::ClickEntity( FEntity* Entity, EMouseButton Button, Integer X, Integer Y )
{
	...
	else if( Button == MB_Right )
	{
		if( Entity->Base->bSelected )
		{
			...
			EntityPopup->Items[12].bEnabled	= Selector.Selected.Num()==1 && Selector.Selected[0]->Script->bHasText;
...
FEntity* WLevelPage::GetEntityAt( Integer X, Integer Y, Bool bFast )
{
	...
	for( Integer i=0; i<Level->Entities.Num(); i++ )
	{
		...
		// Don't hit camera, no way.
		if( Entity == Level->Camera->Entity )
			continue;
...
Integer WLevelPage::GetSocketAt( Integer X, Integer Y, ELogicSocket& SType, FLogicComponent*& L )
{
	...
	for( Integer i=0; i<Level->LogicElements.Num(); i++ )
	{
		FLogicComponent* Test = Level->LogicElements[i];
...
void WLevelPage::DrawLogicCircuit( CCanvas* Canvas )
{
	...
	for( Integer iLog=0; iLog<Level->LogicElements.Num(); iLog++ )
	{
		FLogicComponent*	Logic = Level->LogicElements[iLog];
...
void WLevelPage::DrawScrollClamp( CCanvas* Canvas )
{
	Canvas->DrawLineRect
	(
		Level->ScrollClamp.Center(),
		Level->ScrollClamp.Size(),
		0,
		COLOR_LimeGreen,
		false
	);
...
public:
	...
	CBufferReader( TArray<Byte>& InBuffer )
		:	Buffer( InBuffer ),
			iPos( 0 )
	{
		Mode	= SM_Load;
...
void WLevelPage::PopPasteClick( WWidget* Sender )
{
	...
	Transactor->TrackEnter();
	{
		...
		CBufferReader Reader(GEntityClipboard);
		...
		{
			Entity->InstanceBuffer = new CInstanceBuffer(Script);
...
void WLevelPage::PopDuplicateClick( WWidget* Sender )
{
	Transactor->TrackEnter();
	{
		for( Integer iSource=0; iSource<Selector.Selected.Num(); iSource++ )
		{
			...
			{
				New->InstanceBuffer = new CInstanceBuffer( New->Script );
