...
// Draw GUI picture.
//
void CGUIRender::DrawPicture( TPoint P, TSize S, TPoint BP, TSize BS, FBitmap* Bitmap )
{	
	// Setup rect.
	TRenderRect Rect;
	Rect.Bitmap		= Bitmap;
	...
	// Texture coords.
	Rect.TexCoords.Min.X	= BP.X * Rescale[Bitmap->UBits];
	Rect.TexCoords.Min.Y	= BP.Y * Rescale[Bitmap->VBits];
	Rect.TexCoords.Max.X	= (BP.X+BS.Width)  * Rescale[Bitmap->UBits];
	Rect.TexCoords.Max.Y	= (BP.Y+BS.Height) * Rescale[Bitmap->VBits];
