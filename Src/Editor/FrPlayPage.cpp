...
void WPlayPage::TickPage( Float Delta )
{
	...
	if( PlayMode == PLAY_Debug )
	{
		if( Errors.Num()>0 && (GPlat->Now()-LastPushTime)>7.0 )
		{
			LastPushTime	= GPlat->Now();
			Errors.RemoveShift( 0 );
		}
	}
...
void WPlayPage::RenderPageContent( CCanvas* Canvas )
{
	...
	if( PlayMode == PLAY_Debug )
	{
		for( Integer iErr=0; iErr<Errors.Num(); iErr++ )
		{
			Canvas->DrawText
						( 
							Errors[iErr], 
							Root->Font2, 
							COLOR_White, 
							TVector( 10.f, Size.Height - (Root->Font2->Height+2)*(iErr+1) )
						);
		}
	}
}
...
//
void WPlayPage::AddScriptError( String Message )
{
	if( PlayMode == PLAY_Debug )
	{
		Errors.Push( Message );
		...
		// Check for overflow.
		if( Errors.Num() > MAX_SCRIPT_ERR_LIST )
			Errors.RemoveShift( 0 );
	}
}
...
void WPlayPage::RunLevel()
{
	...
	if( !PlayLevel->Navigator )
		AddScriptError(L"Level has no navigator");
}
