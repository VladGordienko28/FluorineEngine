...
void WGameBuilderDialog::BuildGame()
{
	...
	{
		String ExeDir = GDirectory+L"\\Flu.exe";

		if( !GPlat->FileExists(ExeDir) )
		{
			Root->ShowMessage( L"Couldn't run game. 'Flu.exe' not found.", L"Fluorine", true );
			return;
		}
		GPlat->Launch
		(
			*ExeDir,
			*String::Format( L"\"%s\" %s", *(Directory+L"\\"+GProject->ProjName+PROJ_FILE_EXT), *ParmsEdit->Text )
		);
	}
	else
