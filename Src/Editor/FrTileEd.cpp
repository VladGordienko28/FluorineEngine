...
{
	bAccept = Data && ((FObject*)Data)->IsA(FBitmap::MetaClass) && Editor->Model;
}
...
{
	Editor->Model->Bitmap = (FBitmap*)Data;
}
...
void WTileEditor::WTilesGrid::OnPaint( CGUIRenderBase* Render )
{
	...
	if( Editor->Model && Editor->Model->Bitmap )
	{
		// With model.
		FModelComponent* Model	= Editor->Model;
		...
		Render->DrawPicture
					( 
						...
						Size, 
						TPoint( 0, 0 ), 
						TSize( Model->Bitmap->USize, Editor->Model->Bitmap->VSize ), 
						Model->Bitmap 
					);
