...
#pragma comment (lib, "Audio\\OpenAL\\Lib\\ogg.lib")
#pragma comment (lib, "Audio\\OpenAL\\Lib\\vorbisfile.lib")
...
void COggMusicStream::LoadFromFile( const String& FileName )
{
	...
	{
		log( L"MusicStream: Could not open Ogg stream" );
		return;
	}
...
Bool COggMusicStream::ReadBlock( Integer iBuff )
{
	...
	while( TotalSize < AUDIO_STREAM_BUFFER_SIZE )
	{
		...
		if( Size == 0 )
			...
			log( L"MusicStream: Stream problem" );
...
COpenALAudio::COpenALAudio()
{
	...
	// Notify.
	log( L"OpenAL: OpenAL initialized" );
}
...
{ 
	...
	if( Scene )
	{
		FCameraComponent*	Camera	= Scene->Camera;
		TCoords			Listener	= Camera->ToLocal();
...
void COpenALAudio::RegisterSound( FSound* Sound )
{
	...
	{
		// Header corrupted.
		log( L"OpenAL: Failed register sound '%s', file header corrupted", *Sound->GetName() );
		return;
	}
	...
	{
		// Header corrupted.
		log( L"OpenAL: Failed register sound '%s', wave header corrupted", *Sound->GetName() );
		return;
	}
	...
	while( 1 )
	{
		...
		if( !WaveData.IsValid() )
		{
			...
			if( Timeout-- == 0 )
			{
				log( L"OpenAL: Failed register sound '%s', missing wav data", *Sound->GetName() );
				return;
			}
		}
		else
